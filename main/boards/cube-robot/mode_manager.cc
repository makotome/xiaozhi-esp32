...
ModeManager::ModeManager()
    : current_mode_(kModeXiaozhi), mode_mutex_(nullptr) {}
...
void ModeManager::Initialize() {
  ...
  ESP_LOGI(TAG, "模式管理器初始化完成，当前模式: %s",
           GetModeName(current_mode_));
}

// 获取当前模式
DeviceMode ModeManager::GetCurrentMode() const { return current_mode_; }

// 通知所有监听者模式已改变
void ModeManager::NotifyModeChanged(DeviceMode old_mode, DeviceMode new_mode) {
  ESP_LOGI(TAG, "模式切换: %s -> %s", GetModeName(old_mode),
...
// 注册模式切换回调
void ModeManager::OnModeChanged(
    std::function<void(DeviceMode, DeviceMode)> callback) {
  if (callback) {
