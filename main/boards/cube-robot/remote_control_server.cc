...
RemoteControlServer::RemoteControlServer()
    : server_(nullptr), wheel_controller_(nullptr), is_running_(false) {}
...
// 解析 JSON 请求体
bool RemoteControlServer::ParseJsonBody(httpd_req_t* req, char* buffer,
                                        size_t buffer_size) {
...
// 发送 JSON 响应
esp_err_t RemoteControlServer::SendJsonResponse(httpd_req_t* req,
                                                const char* json) {
...
// 发送错误响应
esp_err_t RemoteControlServer::SendErrorResponse(httpd_req_t* req,
...
esp_err_t RemoteControlServer::HandleStatus(httpd_req_t* req) {
  ...
  snprintf(json, sizeof(json),
           "{\"success\":true,\"mode\":\"remote_control\",\"left_speed\":%d,"
           "\"right_speed\":%d}",
...
esp_err_t RemoteControlServer::HandleCustomSpeed(httpd_req_t* req) {
  ...
  server->wheel_controller_->GetWheels().setWheelSpeeds(left_speed,
                                                        right_speed);
  if (duration > 0) {
    vTaskDelay(pdMS_TO_TICKS(duration));
    server->wheel_controller_->GetWheels().stopAll();
  ...
  ESP_LOGI(TAG, "自定义速度: 左=%d, 右=%d, 持续=%dms", left_speed, right_speed,
           duration);
...
esp_err_t RemoteControlServer::HandleMoveForwardDirection(httpd_req_t* req) {
  ...
  server->wheel_controller_->GetWheels().moveForwardWithDirection(speed,
                                                                  direction);

  ESP_LOGI(TAG, "万向前进: 速度=%d, 方向整数=%d, 方向浮点=%.2f", speed,
           static_cast<int>(direction * 100), direction);
...
esp_err_t RemoteControlServer::HandleMoveBackwardDirection(httpd_req_t* req) {
  ...
  server->wheel_controller_->GetWheels().moveBackwardWithDirection(speed,
                                                                   direction);

  ESP_LOGI(TAG, "万向后退: 速度=%d, 方向整数=%d, 方向浮点=%.2f", speed,
           static_cast<int>(direction * 100), direction);
