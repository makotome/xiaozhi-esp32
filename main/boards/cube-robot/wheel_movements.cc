...
bool WheelMovements::init() {
  ...
  ESP_LOGI(TAG, "→ 创建左轮: GPIO=%d, Channel=%d, Timer=%d", LEFT_HAND_PIN,
           LEFT_WHEEL_CHANNEL, LEFT_WHEEL_TIMER);
  _leftWheel =
      new WheelServo(LEFT_HAND_PIN, LEFT_WHEEL_CHANNEL, LEFT_WHEEL_TIMER);
  ...
  ESP_LOGI(TAG, "→ 创建右轮: GPIO=%d, Channel=%d, Timer=%d", RIGHT_HAND_PIN,
           RIGHT_WHEEL_CHANNEL, RIGHT_WHEEL_TIMER);
  _rightWheel =
      new WheelServo(RIGHT_HAND_PIN, RIGHT_WHEEL_CHANNEL, RIGHT_WHEEL_TIMER);
...
void WheelMovements::moveForward(int speed) {
  ...
  ESP_LOGI(TAG, "  → Left wheel: +%d, Right wheel: -%d (mirrored to forward)",
           speed, speed);
...
void WheelMovements::moveBackward(int speed) {
  ...
  ESP_LOGI(TAG, "  → Left wheel: -%d, Right wheel: +%d (mirrored to backward)",
           speed, speed);
...
void WheelMovements::moveForwardWithDirection(int speed, float direction) {
  ...
  if (direction > 0) {
    ...
    rightSpeed =
        static_cast<int>(speed * (1.0f - direction * DIRECTION_FACTOR));
  } else if (direction < 0) {
    // 左转: 减小左轮速度 (最多减30%,保持70%)
    leftSpeed = static_cast<int>(
        speed * (1.0f + direction * DIRECTION_FACTOR));  // direction是负数
  }

  ESP_LOGI(TAG,
           "Forward with direction: speed=%d, dir=%.2f → left=%d, right=%d",
...
void WheelMovements::moveBackwardWithDirection(int speed, float direction) {
  ...
  ESP_LOGI(TAG, "计算后速度: leftSpeed=%d, rightSpeed=%d", leftSpeed,
           rightSpeed);
...
void WheelMovements::turnLeft(int speed) {
  ...
  ESP_LOGI(TAG, "  → Logical: Left=-%d (backward), Right=+%d (forward)", speed,
           speed);
...
void WheelMovements::turnRight(int speed) {
  ...
  ESP_LOGI(TAG, "  → Logical: Left=+%d (forward), Right=-%d (backward)", speed,
           speed);
  ESP_LOGI(TAG, "  → After mirror: both servos will get +%d PWM signal", speed);
...
void WheelMovements::accelerate(int targetSpeed, int duration_ms) {
  ...
  ESP_LOGI(TAG, "Accelerating from %d to %d over %dms", currentSpeed,
           targetSpeed, duration_ms);
...
void WheelMovements::decelerate(int duration_ms) {
  ...
  if (speedDecrement < 1) speedDecrement = 1;

  ESP_LOGI(TAG, "Decelerating from %d to 0 over %dms", currentSpeed,
           duration_ms);
...
void WheelMovements::setWheelSpeeds(int leftSpeed, int rightSpeed) {
  ...
  ESP_LOGI(TAG, "Set wheel speeds: left=%d, right=%d (logical values)",
  ...
  ESP_LOGI(TAG, "  → Actual servo commands: left=%d, right=%d (after mirror)",
           leftSpeed, -rightSpeed);
}
...
void WheelMovements::speakingGestureShake() {
  ...
  // 缓慢地向右转（速度加快，时间缩短以平衡）
  turnRight(10);
  vTaskDelay(pdMS_TO_TICKS(680));  // 调整为 680ms，确保左右转动量相等 (8×450 +
                                   // 8×400 = 10×680 + 误差补偿)
...
void WheelMovements::performRandomSpeakingGesture() {
  ...
  int64_t min_interval_us =
      (2000 + (esp_random() % 3000)) * 1000;  // 2000-5000ms
