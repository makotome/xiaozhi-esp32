...
```cmake
elseif(CONFIG_BOARD_TYPE_CUBE_ROBOT)
    set(BOARD_TYPE "cube-robot")
...
在菜单中选择:
- `Xiaozhi Assistant` -> `Board Type` -> `cubeRobot`
...
// 2. 在 CubeRobot 类中添加 mode_button_ 成员
private:
    Button boot_button_;
...
在构造函数的初始化列表中,找到:
```cpp
CubeRobot() : boot_button_(BOOT_BUTTON_GPIO)
```
