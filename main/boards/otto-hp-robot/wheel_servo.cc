...
WheelServo::WheelServo(int pin, int channel, int timer)
    : _pin(pin), _channel(channel), _timer(timer), _currentSpeed(0), _initialized(false)
{
}
...
bool WheelServo::init()
{
    ...
    ESP_LOGI(TAG, "Initializing WheelServo on GPIO %d, Channel %d, Timer %d", _pin, _channel, _timer);
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to configure LEDC timer %d: %s", _timer, esp_err_to_name(ret));
        return false;
    }
    ...
    ledc_channel.timer_sel = static_cast<ledc_timer_t>(_timer); // 使用自己的Timer
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to configure LEDC channel on pin %d: %s", _pin, esp_err_to_name(ret));
        ESP_LOGE(TAG, "  → Channel=%d, Timer=%d, GPIO=%d", _channel, _timer, _pin);
        return false;
    }

    ESP_LOGI(TAG, "✓ LEDC Channel %d configured: GPIO=%d, Timer=%d", _channel, _pin, _timer);
    ...
    ESP_LOGI(TAG, "✓ WheelServo initialized successfully on GPIO %d, Channel %d", _pin, _channel);
    return true;
}
...
void WheelServo::setSpeed(int speed)
{
    ...
    ESP_LOGI(TAG, "🔧 [GPIO %d Ch %d T %d] Setting speed=%d, duty=%u", _pin, _channel, _timer, speed, duty);

    esp_err_t ret = ledc_set_duty(LEDC_LOW_SPEED_MODE, static_cast<ledc_channel_t>(_channel), duty);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "❌ ledc_set_duty FAILED for pin %d ch %d: %s", _pin, _channel, esp_err_to_name(ret));
        return;
    }

    ret = ledc_update_duty(LEDC_LOW_SPEED_MODE, static_cast<ledc_channel_t>(_channel));
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "❌ ledc_update_duty FAILED for pin %d ch %d: %s", _pin, _channel, esp_err_to_name(ret));
        return;
    }

    ESP_LOGI(TAG, "✅ [GPIO %d Ch %d T %d] PWM updated: speed=%d, duty=%u", _pin, _channel, _timer, speed, duty);
}

void WheelServo::stop()
{
    setSpeed(0);
}
...
unsigned int WheelServo::speedToDuty(int speed)
{
    ...
    // 简化日志，减少输出
    // ESP_LOGI(TAG, "Speed %d -> Pulse %.3fms -> Duty %u", speed, pulse_ms, duty);
    return duty;
}
...
void WheelServo::runHardwareDiagnostic()
{
    ...
    ledc_set_duty(LEDC_LOW_SPEED_MODE, static_cast<ledc_channel_t>(_channel), 614);
    ...
    ledc_set_duty(LEDC_LOW_SPEED_MODE, static_cast<ledc_channel_t>(_channel), 819);
    ledc_update_duty(LEDC_LOW_SPEED_MODE, static_cast<ledc_channel_t>(_channel));
    vTaskDelay(pdMS_TO_TICKS(2000));
    ...
    ledc_set_duty(LEDC_LOW_SPEED_MODE, static_cast<ledc_channel_t>(_channel), 409);
    ledc_update_duty(LEDC_LOW_SPEED_MODE, static_cast<ledc_channel_t>(_channel));
    vTaskDelay(pdMS_TO_TICKS(2000));
