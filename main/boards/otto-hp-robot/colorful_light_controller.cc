...
void ColorfulLightController::setLightMode(LightMode mode)
{
    ...
    {
        is_running_ = true;
        xTaskCreate(LightEffectTask, "light_effect", 4096, this, 5, &light_task_handle_);
    }
}
...
void ColorfulLightController::createLightOverlay()
{
    ...
    {
        ESP_LOGW(TAG, "无法创建灯光覆盖层: effect_active_=%d, display_=%p", effect_active_, display_);
        return;
    }
    ...
    lv_obj_move_to_index(light_overlay_, -1); // 移到父对象的最后一个子对象位置（最顶层）
    ...
    ESP_LOGI(TAG, "灯光覆盖层创建成功: overlay=%p, parent=%p", light_overlay_, lv_obj_get_parent(light_overlay_));
}
...
void ColorfulLightController::setOverlayColor(lv_color_t color, uint8_t opacity)
{
    ...
    {
        ESP_LOGW(TAG, "setOverlayColor失败: effect_active_=%d, light_overlay_=%p, display_=%p",
    ...
    ESP_LOGI(TAG, "设置覆盖层颜色: R=%d G=%d B=%d, 透明度=%d",
...
{
    ColorfulLightController *controller = static_cast<ColorfulLightController *>(arg);
